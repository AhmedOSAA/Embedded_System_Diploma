...
typedef struct {
	EXTI_GPIO_Mapping_t EXTI_PIN ;		// Specifies the External inturrupt GPIO mapping.
										// This parametar must be set based on @ref EXTI_define

	uint8_t				Trigger_Case;	// Specifies Rising or Falling or Both trigger.
										// This parametar must be set based on @ref EXTI_Trigger_define

	uint8_t				IRQ_EN;			// Enable or desable th EXTI IRQ (That will enable th IRQ Mask in EXTI and Also on the NVIC Interrupt controller)
										// This parametar must be set based on @ref EXTI_IRQ_define
