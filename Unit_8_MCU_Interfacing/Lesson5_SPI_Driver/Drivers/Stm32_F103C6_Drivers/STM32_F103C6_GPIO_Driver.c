...
/*=====================================================================
 * @Fun			- MCAL_GPIO_WritePort
 * @brief		- Partial Write on the output port
 * @param [in] 	- GPIOx: where x can be (A..E depending on device used) to select the GPIO peripheral
 * @param [in] 	- Value: Port partial value
 * @param [in] 	- NUMofBIT: Number of Value Bits will be over written @ref : GPIO_PARTIAL_PORT_define
 * @param [in] 	- shift: write what you want to shift the value by number
 * @retval		- none
 * Note			- none
 */
...
void MCAL_GPIO_WriteINPort (GPIO_typedef* GPIOx , uint16_t Value, int NUMofBIT, int shift){
	...
	GPIOx->ODR &= ~(NUMofBIT<<shift) ;
	GPIOx->ODR |= (uint32_t)(Value<<shift) ;
